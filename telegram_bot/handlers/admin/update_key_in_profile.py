    ...
    async with DatabaseContextManager() as session_methods:
        try:
            ...
            for sub in subscriptions:
                try:
                    result = await _process_subscription(session_methods, sub, existing_key_ids)
                    if result:
                        processed_count += 1
                except Exception as e:
                    error_count += 1
                    # ✅ Используем subscription_id вместо user_id
...
async def _process_subscription(session_methods, subscription, existing_key_ids) -> bool:
    """
    Обрабатывает одну подписку.

    Returns:
        bool: True если подписка была обновлена, False если не требовалось обновление
    """
    # ✅ Извлекаем все нужные данные из модели ДО входа в try блок
    ...
    try:
        ...
        # Обновляем подписку - добавляем новые ключи к существующим
        updated_key_ids = current_key_ids + new_key_ids

        # ✅ Используем правильный метод update_sub
        ...
        # ✅ Используем сохранённые переменные вместо обращения к модели
...
async def _create_server_keys(session_methods, user_id: int, username: str) -> list[int]:
    ...
    Returns:
        list[int]: Список ID созданных ключей
    """
    created_key_ids = []
